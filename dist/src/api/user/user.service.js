...
const addUser = async ({ name, email, password, }) => {
    try {
        ...
        logger_js_1.default.error("Error adding user:", error);
...
const getUsers = async () => {
    try {
        ...
        logger_js_1.default.error("Failed to fetch users:", error);
...
const getUserById = async (id) => {
    try {
        return await user_js_1.default.findById(id).select("-password").lean();
    }
    catch (error) {
        logger_js_1.default.error("Error getting user by ID:", error);
        throw new errors_js_1.CustomException(500, "Failed to get user by ID");
    }
};
exports.getUserById = getUserById;
const getUserByEmail = async (email) => {
    try {
        return await user_js_1.default.findOne({ email })
            .select(["-password", "-refreshToken"])
            .lean();
    }
    catch (error) {
        logger_js_1.default.error("Error getting user by email:", error);
        throw new errors_js_1.CustomException(500, "Failed to get user by email");
    }
};
exports.getUserByEmail = getUserByEmail;
const updateUser = async ({ id, name, email, role, password, refreshToken, }) => {
    try {
        const updatedUser = await user_js_1.default.findByIdAndUpdate(id, {
            ...(name && { name }),
            ...(email && { email }),
            ...(role && { role }),
            ...(password && { password }),
...
const deleteUserById = async (id) => {
    try {
        return await user_js_1.default.findByIdAndDelete(id);
    }
    catch (error) {
