...
const createTask = async ({ title, description, project, }) => {
    try {
        const task = new task_1.default({ title, description, project });
        await task.save();
        return task.toObject();
    }
    catch (error) {
        logger_1.default.error("Error adding task:", error);
...
const getTasks = async (projectId) => {
    try {
        const tasks = await task_1.default.find({ projectId }).lean();
        return tasks;
    }
    catch (error) {
        logger_1.default.error("Failed to fetch tasks:", error);
        return [];
    }
};
...
const getTaskById = async (id) => {
    try {
        return await task_1.default.findById(id).lean();
    }
    catch (error) {
        logger_1.default.error("Error getting task by ID:", error);
        throw new errors_1.CustomException(500, "Failed to get task by ID");
    }
};
exports.getTaskById = getTaskById;
const getTaskByTitle = async (name) => {
    try {
        return await task_1.default.findOne({ name }).select(["-members"]).lean();
    }
    catch (error) {
        logger_1.default.error("Error getting task by title:", error);
        throw new errors_1.CustomException(500, "Failed to get task by title");
    }
};
...
const deleteTaskById = async (id) => {
    try {
        return await task_1.default.findByIdAndDelete(id);
    }
    catch (error) {
