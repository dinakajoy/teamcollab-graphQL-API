...
const createProject = async ({ name, description, team, }) => {
    try {
        const project = new project_1.default({ name, description, team });
        await project.save();
        return project.toObject();
    }
    catch (error) {
        logger_1.default.error("Error adding project:", error);
...
const getProjectById = async (id) => {
    try {
        ...
        throw new errors_1.CustomException(500, "Failed to get project by ID");
    }
};
exports.getProjectById = getProjectById;
const getProjectByName = async (name) => {
    try {
        return await project_1.default.findOne({ name })
            .select(["-members"])
            .lean();
    }
    catch (error) {
        logger_1.default.error("Error getting project by name:", error);
        throw new errors_1.CustomException(500, "Failed to get project by name");
    }
};
